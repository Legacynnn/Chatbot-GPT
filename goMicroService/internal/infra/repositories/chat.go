...
import (
	...
	"github.com/Legacynnn/Chatbot-GPT/goMicroService/internal/infra/db"
)
...
func (r *ChatRepositoryMySQL) CreateChat(ctx context.Context, chat *entities.Chat) error {
	err := r.Queries.CreateChat(
		...
		db.CreateChatParams{
			ID:               chat.ID,
			UserID:           chat.UserID,
			InitialMessageID: chat.InitialSystemMenssage.Content,
			Status:           chat.Status,
	...
	err = r.Queries.AddMessage(
		ctx,
		db.AddMessageParams{
			ID:        chat.InitialSystemMenssage.ID,
			ChatID:    chat.ID,
			Content:   chat.InitialSystemMenssage.Content,
			Role:      chat.InitialSystemMenssage.Role,
			Tokens:    int32(chat.InitialSystemMenssage.Tokens),
			CreatedAt: chat.InitialSystemMenssage.CreatedAt,
		},
	)
	if err != nil {
		return err
...
func (r *ChatRepositoryMySQL) FindChatById(ctx context.Context, chatID string) (*entities.Chat, error) {
	chat := &entities.Chat{}
	res, err := r.Queries.FindChatByID(ctx, chat.ID)
	if err != nil {
